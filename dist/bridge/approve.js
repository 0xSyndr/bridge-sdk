...
import { PARAKEET_BRIDGE_ADDRESS } from '../constants';
import { getContract } from '../utils';
export const approve = async ({ srcChainId, nftCollectionAddress, tokenId, provider, account, }) => {
    const _provider = new Web3Provider(provider);
    try {
        const bridge = await getContract(PARAKEET_BRIDGE_ADDRESS[srcChainId], BridgeContract.abi, _provider, account);
        const NFT = await getContract(nftCollectionAddress, BridgedERC721.abi, _provider, account);
