...
export const estimateBridgeFees = async ({
  ...
  try {
    ...
    const lzEndpoint = await getContract(
      endpointAddress,
      LayerZeroEndpoint.abi,
      web3Provider
    );

    const payload = abiCoder.encode(
      ['string', 'string', 'string', 'uint', 'uint'],
      [
        account,
        nftSrcContractAddress,
    ...
    const res = await lzEndpoint.estimateFees(
      lzDstChainId,
      bridgeAddr,
      payload,
      false,
  ...
  for (const [srcId, srcName] of CHAIN_MAPPABLE) {
    ...
    for (const [dstId, dstName] of CHAIN_MAPPABLE) {
      let estimatedFee = await estimateBridgeFees({
        ...
        dstChainId: dstId,
        nftSrcContractAddress:
          WHITELISTED_COLLECTIONS['MFNFT'].contracts[srcId],
        tokenId: 535668,
        provider: PROVIDERS[srcId],
      });
      bridgeFees[srcId][dstId] = formatEther(
        estimatedFee.nativeFee.mul(parseEther('1.1')).div(parseEther('1'))
      );
      console.log(
        dstName,
        formatEther(
        ...
        formatEther(estimatedFee.nativeFee)
      );
    }
  }
